...
void copyDirRecursive(const fs::path &src, const fs::path &dest,
    ...
    for (const auto &entry : fs::recursive_directory_iterator(absSrc)) {
        ...
        fs::path relative = fs::relative(path, absSrc);
        std::string r = relative.string();

        // Skip .git and .minigit including all children
        if (skipMeta) {
            if (r == ".git" || startsWith(r, ".git/") || r == ".minigit" ||
                startsWith(r, ".minigit/"))
                continue;
        }

        fs::path target = absDest / relative;
